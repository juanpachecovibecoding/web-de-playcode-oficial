...
export const FileManager: React.FC<FileManagerProps> = ({ files, setFiles }) => {
  ...
  /** Build a clean public URL: playcode.com.ar/archivos/folder1/folder2/filename */
  const getPublicUrl = (file: VirtualFile): string => {
    ...
    return `${window.location.origin}/archivos/${path}`;
  };

  const handleCopyPublicLink = (file: VirtualFile) => {
    const url = getPublicUrl(file);
    navigator.clipboard.writeText(url);
    setCopiedPublicId(file.id);
                      ...
                      <div className="absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                        ...
                        {item.type === 'file' && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleCopyPublicLink(item);
                            }}
                            className={`p-1 border rounded text-[10px] font-bold flex items-center gap-0.5 ${
                              copiedPublicId === item.id
                          ...
                          >
                            {copiedPublicId === item.id ? (
                              ...
                              <><ExternalLink className="w-3 h-3" /><span>Link</span></>
                            )}
                          </button>
                        )}
                        {item.type === 'file' && (
